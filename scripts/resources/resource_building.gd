...
func setup(gx, gy, lv = 1):
    ...
    print("[TEX_LOAD] 资源建筑纹理: ", tex_path, " 存在=", ResourceLoader.exists(tex_path))
    _sprite.texture = load(tex_path)
    print("[TEX_LOAD]   资源建筑纹理加载: ", _sprite.texture != null, " 尺寸=", _sprite.texture.get_width() if _sprite.texture else -1, "x", _sprite.texture.get_height() if _sprite.texture else -1)
